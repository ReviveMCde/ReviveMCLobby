...
public class DeluxeCooldown {
    ...
    public static Long getTime(UUID uuid) {
        ...
        if (playerExists(uuid)) {
            try {
                ResultSet rs = Lobby.getInstance().getDatabaseDriver().query("SELECT * FROM deluxecooldown WHERE UUID= '" + uuid + "'");
                if (rs.next()) {
    ...
    public static void setTime(UUID uuid, String time) {
        if (playerExists(uuid)) {
            Lobby.getInstance().getDatabaseDriver().update("UPDATE deluxecooldown SET time = '" + time + "' WHERE UUID= '" + uuid + "';");
        }
    }
