...
public class PremiumCooldown {
    ...
    public static Long getTime(UUID uuid) {
        ...
        if (playerExists(uuid)) {
            try {
                ResultSet rs = Lobby.getInstance().getDatabaseDriver().query("SELECT * FROM premiumcooldown WHERE UUID= '" + uuid + "'");
                if (rs.next()) {
    ...
    public static void setTime(UUID uuid, String time) {
        if (playerExists(uuid)) {
            Lobby.getInstance().getDatabaseDriver().update("UPDATE premiumcooldown SET time = '" + time + "' WHERE UUID= '" + uuid + "';");
        }
    }
